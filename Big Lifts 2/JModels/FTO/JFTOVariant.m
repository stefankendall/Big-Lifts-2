#import "JFTOVariant.h"

NSString *const FTO_VARIANT_STANDARD = @"Standard";
NSString *const FTO_VARIANT_HEAVIER = @"Heavier";
NSString *const FTO_VARIANT_POWERLIFTING = @"Powerlifting";
NSString *const FTO_VARIANT_SIX_WEEK = @"6 Week";
NSString *const FTO_VARIANT_PYRAMID = @"Pyramid";
NSString *const FTO_VARIANT_JOKER = @"Joker";
NSString *const FTO_VARIANT_FIRST_SET_LAST_MULTIPLE_SETS = @"First Set Last Multiple Sets";
NSString *const FTO_VARIANT_FIRST_SET_LAST = @"First Set Last";
NSString *const FTO_VARIANT_ADVANCED = @"Advanced";
NSString *const FTO_VARIANT_FIVES_PROGRESSION = @"Fives Progression";
NSString *const FTO_VARIANT_CUSTOM = @"Custom";

@implementation JFTOVariant

@end